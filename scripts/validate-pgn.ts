...
function validate(): boolean {
  ...
  for (const relPath of pgnFiles) {
    ...
    for (let i = 0; i < games.length; i++) {
      const game = games[i];
      // Skeleton PGNs (generated by import-playlist, not yet annotated) have Result "*"
      const isSkeleton = game.headers['Result'] === '*';
      const missing = REQUIRED_HEADERS.filter((h) => !game.headers[h]);
      const placeholders = isSkeleton ? [] : NO_PLACEHOLDER.filter((h) => game.headers[h] === '?');
